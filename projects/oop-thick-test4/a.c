...
# 7 "GDefines.h" 2
# 1 "/usr/lib/gcc/aarch64-linux-gnu/12/include/stdbool.h" 1 3 4
# 8 "GDefines.h" 2
...
extern void *THIS;
# 16 "GObj.h"
typedef struct GObj GObj; typedef struct GObj { char name[16]; GObj * (*init)(); GObj * (*delete)(); void (*debug)(char *message); char * (*toString)(); } GObj;

extern GObj *GObj_new(char *name);
# 5 "GCron.h" 2
# 1 "GList.h" 1
# 15 "GList.h"
typedef struct GList GList; typedef struct GList { char name[16]; GObj * (*init)(); GObj * (*delete)(); void (*debug)(char *message); char * (*toString)(); void **array; int extent; int size; double mult; void (*push)(void *value); void * (*get)(int n); } GList;

GList *GList_new(char *name, int extent, double mult);
# 6 "GCron.h" 2
# 1 "GCronEntry.h" 1
# 13 "GCronEntry.h"
typedef struct GCronEntry GCronEntry; typedef struct GCronEntry { char name[16]; GObj * (*init)(); GObj * (*delete)(); void (*debug)(char *message); char * (*toString)(); int msRepeat; int msLast; int flags; void *context; int (*callback)(GCronEntry *self); } GCronEntry;

extern GCronEntry *GCronEntry_new(char * name, int msRepeat, int (*callback)(GCronEntry *self), void *context);
# 7 "GCron.h" 2
# 16 "GCron.h"
typedef struct GCron GCron; typedef struct GCron { char name[16]; GObj * (*init)(); GObj * (*delete)(); void (*debug)(char *message); char * (*toString)(); GList *list; int (*pump)(); GCron * (*add)(char *, int msRepeat, int (*callback)(GCronEntry *), void *context); } GCron;
...
static GCron *add(char * name, int msRepeat, int (*callback)(GCronEntry *), void *context)
{
    ...
    (THIS=this->list, this->list)->push(entry);
    return this;
}
# 38 "<stdin>"
static void debug(char * message)
{
    ...
    printf("DEBUG: %s\n", message);
    printf("DEBUG: this=%p\n", this);
    printf("DEBUG: _super=%p\n", _super);

    (THIS=this->list, this->list)->debug("list");
}

static void delete()
{
    GCron *this = THIS;
    (THIS=this->list, this->list)->delete();
    (THIS=this, _super)->delete();
}

static GCron *init(char *name)
{
    GCron *this = THIS;
    (THIS=this, _super)->init(name);
    this->delete = delete;
    this->debug = debug;
    this->add = add;
...
GCron *GCron_new(char * name) {
    ...
    GCron *this = (GCron *) calloc(1, sizeof(GCron));
    this->init = init;
