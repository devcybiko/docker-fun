...
class LCD_1inch28(framebuf.FrameBuffer):
    ...
    def write_text(self,text,x,y,size,color):
        ...
        for px_info in info:
            self.fill_rect(round(size*px_info[0]) - round((size-1)*x) , round(size*px_info[1] - (size-1)*y), round(size), round(size), px_info[2])
        return [x,round(size*8+y)+1]
...
def DOF_READ():
    ...
    while(True):
        ...
        Face = max([min([Face,1]), -1])
        Top = max([min([Top,1]), -1])
        Side = max([min([Side,1]), -1])
        ...
        nn = 3
        if len(face) > nn: face.pop(0)
        if len(top) > nn: top.pop(0)
        if len(side) > nn: side.pop(0)
        
        Face = sum(face)/len(face)
        Top = sum(top)/len(top)
        Side = sum(side)/len(side)
        
        
#         LCD.fill_rect(0,0,240,40,LCD.red)
#         LCD.write_text("Roll Over",50,25,FONT_SIZE, LCD.white)
#         
#         LCD.fill_rect(0,40,240,40,LCD.blue)
#         # LCD.text("Long Press to Quit",20,57,LCD.white)
#         LCD.write_text("By Greg Smith",20,57,FONT_SIZE,LCD.white)
        ...
        if abs(Side) < SideThresh or abs(Top) > TopThresh or abs(Face) > FaceThresh: Bgnd=Bad
        LCD.fill_rect(0,0,240,240,Bgnd)
        [X,Y] = LCD.write_text("Top={:+06.2f}".format(Top*100),X,Y, FONT_SIZE,LCD.white)
        [X,Y] = LCD.write_text("Side={:+06.2f}".format(Side*100),X,Y, FONT_SIZE,LCD.white)
        [X,Y] = LCD.write_text("Face={:+06.2f}".format(Face*100),X,Y, FONT_SIZE,LCD.white)

        # LCD.fill_rect(120,80,120,120,0xF073)
        # X,Y] = LCD.write_text("ACC_X={:+04.0f}".format(xyz[3]*100),X,Y,FONT_SIZE, LCD.white)
...
#         LCD.text("Vbat={:.2f}".format(reading),80,215,LCD.white)
        ...
        # cr = round(120 - Face * 120)
        cr = 10
        cy = round(120 + Top * 120)
        cx = round(120 - Side * 120)
        LCD.fill_rect(round(cx-cr/2), round(cy-cr/2), cr, cr, LCD.black)
